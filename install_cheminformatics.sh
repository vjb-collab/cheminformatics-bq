...
gcloud functions deploy rdkit-pattern-fingerprint --entry-point rdkit_pattern_fingerprint --runtime python39 \
    --trigger-http --quiet --memory=512MB --timeout=240s > /dev/null

CLOUD_TRIGGER_URL=$(gcloud functions describe rdkit-pattern-fingerprint --format=json | jq -r '.httpsTrigger.url')
